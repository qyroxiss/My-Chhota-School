...
export default async function PaperPreviewPage(props: PageProps<'/papers/[id]/preview'>) {
  ...
  const isMultiSubject = content.questions.some(q => q.sectionLabel)
      ...
      <div className="paper-body max-w-3xl mx-auto my-8 bg-white p-12 shadow-sm rounded-2xl">
        ...
        <div className="flex justify-between items-start mb-6 text-sm">
          <div className="space-y-1">
            ...
            {!isMultiSubject && <p><span className="font-semibold text-slate-700">Subject:</span> <span className="text-slate-900">{content.subject}</span></p>}
          </div>
        ...
        {/* First section header for multi-subject papers */}
        {isMultiSubject && (
          <SectionHeader
            ...
            <div key={idx}>
              ...
              {q.sectionLabel && (
                <SectionHeader
                  ...
                  maxMarks={q.sectionLabel.maxMarks}
                  duration={q.sectionLabel.duration}
                />
              )}

              {/* Subsection header (Roman numeral section) */}
              {q.subsectionLabel && (
...
function SectionHeader({ subject, maxMarks, duration }: { subject: string; maxMarks: number; duration: string }) {
  return (
    <div className="flex items-center justify-between border-t-2 border-b border-slate-800 mt-8 mb-4 py-2">
      <p className="font-bold text-slate-900 uppercase text-sm tracking-wide">Subject: {subject}</p>
      <div className="text-right text-sm space-x-4">
        {maxMarks > 0 && <span><span className="font-semibold text-slate-700">M.M.:</span> <span className="text-slate-900">{maxMarks}</span></span>}
        {duration && <span><span className="font-semibold text-slate-700">Time:</span> <span className="text-slate-900">{duration}</span></span>}
      </div>
    </div>
  )
}
